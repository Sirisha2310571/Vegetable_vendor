...
st.set_page_config(
    page_title="WELCOME TO RAM STORES", 
    page_icon="c:\\sem2\\icon.jpeg"
)
...
class Graph:
    ...
    def bfs(self, start, end):
        ...
        while queue:
            ...
            for next in set(self.get_neighbors(vertex)) - set(path):
                if next == end:
                    yield path + [next]
                else:
...
def customer_signup():
    ...
    if st.button("Sign Up"):
        if not customer_exists(customer_username):
            ...
            st.session_state.page = "customer_login"
            send_email(customer_username, "Welcome to Online Door Delivery Vegetable Shopping", "Thank you for signing up!")
        else:
            st.error("User already exists!")
    back_button("customer_login")
...
def update_inventory_after_sale(cart):
    ...
    for item in cart:
        ...
        if veg_name in inventory:
            inventory[veg_name]['quantity'] -= quantity_sold
...
def process_payment(bill, payment_method):
    payment_graph = Graph()
    payment_graph.add_edge("Pending", "Paid")
    payment_graph.add_edge("Pending", "Failed")

    if payment_method == "Cash on Delivery":
        payment_status = "Paid"
    else:
        payment_status = "Paid"  # Assuming successful payment via GPay

    if payment_status in payment_graph.get_neighbors("Pending"):
        bill['Status'] = payment_status
        bill['payment_method'] = payment_method
        bills_queue = read_bills()
        write_bills(bills_queue)  # Ensure bills_queue is updated with the modified bill
...
def generate_bill(customer_username, cart, location):
    def display_bill_using_hash_table(cart):
        ...
        # Update inventory after generating bill
...
def generate_bill_id():
    return str(uuid.uuid4())[:8]  # Return the first 8 characters of a UUID as a string
...
def checkout(cart=None):
    ...
    if not st.session_state.cart.is_empty() and 'location' in st.session_state:
        ...
        if bill:
            # Ensure the bill has an 'id'
            if 'id' not in bill:
                bill['id'] = generate_bill_id()  # Assuming you have a function to generate a unique bill ID
            ...
            if st.button("Proceed for Payment"):
    ...
    back_button("customer_dashboard")
...
def remove_duplicates():
    try:
        ...
        print("File not found. Proceeding with an empty data structure.")
        data = []
    l=[]
    for i in data:
        if i not in l:
...
def main():
    ...
    if st.session_state.page == "welcome":
        ...
        if 'bill' in st.session_state:
            payment_page(st.session_state.bill)
        else:
            st.error("No bill found. Please try again.")
    remove_duplicates()
